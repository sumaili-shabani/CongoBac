...
$(document).ready(function(){
 ...
 {
  $.ajax({
   url:"<?php echo base_url(); ?>instructor/pagination_access_formation_to_learn/"+page,
   method:"GET",
   dataType:"json",
 ...
 {
	  $.ajax({
	   url:"<?php echo base_url(); ?>instructor/fetch_search_formation_to_learn",
	   method:"POST",
	   data:{query:query},
	   success:function(data){
 ...
 $(document).ready(function(){  
      ...
      $(document).on('click', '.inscription', function(event){  
           ...
           if(iduser != '' && idformation != '')
            {
        		$.ajax({  
                     url:"<?php echo base_url() . 'admin/view_formation/inscription'?>",  
                     method:'POST',  
                     data:{
                     	iduser:iduser,
