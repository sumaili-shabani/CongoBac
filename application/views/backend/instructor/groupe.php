                                ...
                                <div class="mail-side">
                                    ...
                                    <ul class="nav">
                                    	...
                                    	foreach ($gr as $chat) {
                                    		?>
                                    		<li>
                                    			<a href="<?php echo(base_url()) ?>instructor/chat_admin2/<?php echo($id) ?>/<?php echo($chat['code']) ?>">
                                    	...
                                    	if ($gr->num_rows() > 0) {
                                			...
                                			<li class="clearfix">
	                                            ...
	                                            <div class="md-right">
	                                                ...
	                                                <p class="sub"><a href="<?php echo(base_url()) ?>instructor/chat_admin/<?php echo($id) ?>/<?php echo($key['id']) ?>" class="mail-detail-expand"><?php echo($key['first_name']) ?> <?php echo($key['last_name']) ?></a></p>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form method="post" id="user_form" action="<?php echo(base_url()) ?>instructor/operation_goupe" enctype="multipart/form-data">
...
<script type="text/javascript">
  $(document).ready(function(){
	  $(document).on('keyup', '#search_text', function(){
	    ...
	    $.ajax({
	      url:"<?php echo(base_url()) ?>instructor/recherche_utilisateur_groupe",
	      method: 'POST',
	      data: {
	        query: query
	      },
	  ...
	  $(document).on('click', '.viw_groupe', function(){
	    ...
	    $.ajax({
	      url:"<?php echo(base_url()) ?>instructor/nos_goupe_discution_groupe",
	      method: 'POST',
	      success: function(data){
	        format.html(data);
	      }
  			   ...
  			   swal({
  			        ...
  			        if (isConfirm) {
  			        	// swal("id_groupe:", ""+checkbox_value,"info");
      					   $.ajax({
        					    url:"<?php echo base_url(); ?>instructor/delete_groupe",
        					    method:"POST",
	  	  ...
	  	  if(checkbox.length > 0 && code_groupe.length > 0)
		  {
			   ...
			   swal({
			        ...
			        if (isConfirm) {
					   ...
					   $.ajax({
					    url:"<?php echo base_url(); ?>instructor/integration_du_groupe",
					    method:"POST",
					    data:{
	  	  ...
	  	  if(checkbox.length > 0 && code_groupe.length > 0)
		  {
			   ...
			   swal({
			        ...
			        if (isConfirm) {
			        	// swal("id_groupe:", ""+code_groupe_value+" idusers:"+checkbox_value,"info");
					   $.ajax({
					    url:"<?php echo base_url(); ?>instructor/retirer_integration_du_groupe",
					    method:"POST",
					    data:{
					   			checkbox_value:checkbox_value,
					   			code_groupe_value:code_groupe_value
	...
	$(document).on('dblclick','.show_member_of_group', function(){
	    ...
	    $.ajax({
	      url:"<?php echo(base_url()) ?>instructor/profile_groupe",
	      method: 'POST',
	      data: {
	        code: code
	      },
