                ...
                <div class="col-lg-12 col-md-8 col-sm-12">
                    <div class="row clearfix">
                    	...
                    	<?php
                             ...
                             $message = $this->db->query("SELECT idmessage,id_user,id_recever, messagerie.created_at, users.first_name,users.last_name, users.image, message FROM messagerie INNER JOIN users ON  users.id= messagerie.id_user WHERE messagerie.id_recever = '".$id."'  ORDER BY messagerie.created_at DESC LIMIT 16 ");
                                ...
                                <div class="col-lg-3 col-md-6">
		                            <div class="card c_grid c_yellow">
		                                <div class="body text-center">
		                                    <div class="thumbnail">
		                                        <img class="img-thumbnail" src="<?php echo(base_url()) ?>upload/photo/<?php echo($not['image']) ?>" alt="" style="height: 100px;">
		                                    </div>
		                                    <h6 class="mt-3 mb-0"><span class="text-info"><?php echo($not['first_name']); ?> </span><?php echo(substr($not['message'], 0,20)); ?>... </h6>

		                                    
		                                    <a href="<?php echo(base_url()) ?>instructor/chat_admin/<?php echo($id) ?>/<?php echo($not['id_user']) ?>" class="btn btn-default btn-sm"><i class="fa fa-comment-o"></i>chat</a>

		                                    <a onclick="return confirm('Etes-vous sûre de vouloire Supprimer ce message?')" href="<?php echo(base_url()) ?>instructor/view/display_delete_message/<?php echo($id) ?>/<?php echo($not['idmessage']) ?>" class="btn btn-default btn-sm"><i class="fa fa-trash"></i> supprimer</a>
		                                </div>
		                            </div>
		                        </div>
