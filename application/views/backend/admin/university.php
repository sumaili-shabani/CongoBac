		                      ...
		                      <table id="user_data" class="table table-hover table-custom spacing5 ">  
		                       <thead>  
		                            <tr>  
		                            	 ...
		                            	 <th width="40%">Nom de l'université</th> 
            ...
            <div class="modal-header">
             ...
             <form method="post" id="user_form" enctype="multipart/form-data">
	            <div class="modal-body">
                    ...
                    <div class="input-group mb-3">
	                    <input type="text" name="nom" id="nom" class="form-control" placeholder="nom de l'université">
	                </div>
	                <div class="input-group mb-3">
	                     <input type="url" name="lien" id="lien" class="form-control" placeholder="https://domaine.com">
	                </div>

	                <div class="input-group mb-3">
	                    <textarea name="description" id="description" class="form-control" placeholder="une petite description de l'université">
...
<script type="text/javascript" language="javascript" >  
 $(document).ready(function(){  
      $('#add_button').click(function(){  
           $('#user_form')[0].reset();  
           $('.modal-title').text("Ajout d'une université");  
           $('#action').val("Add");  
      });
      ...
      var dataTable = $('#user_data').DataTable({  
           ...
           "order":[],  
           "ajax":{  
                url:"<?php echo base_url() . 'admin/fetch_university'; ?>",  
                type:"POST"  
           },  
           "columnDefs":[  
                {  
      ...
      $(document).on('submit', '#user_form', function(event){  
           ...
           if(nom != '' && idv !='' && description !='' && lien !='')
            {

	            if (action =="Add") {
	                 
	                $.ajax({  
	                     url:"<?php echo base_url() . 'admin/operation_university/Add'?>",  
	                     method:'POST',  
		        	...
		        	$.ajax({  
                         url:"<?php echo base_url() . 'admin/modification_university'?>",  
                         method:'POST',  
                         data:new FormData(this),  
                         contentType:false,  
                         processData:false,  
      ...
      $(document).on('click', '.update', function(){  
           var iduniv = $(this).attr("iduniv");  
           $.ajax({  
                url:"<?php echo base_url(); ?>admin/fetch_single_university",  
                method:"POST",  
                data:{iduniv:iduniv},  
                dataType:"json",  
                success:function(data)  
                {  
                     ...
                     $('#idv').val(data.idv);
                     $('#lien').val(data.lien);
                     $('.modal-title').text("modification de l'université");  
                     $('#iduniv').val(iduniv);  
                     $('#action').val("Edit");  
                }  
           });  
          ...
          var iduniv = $(this).attr("iduniv");
          	...
          	swal({
		        ...
		        if (isConfirm) {
		        	...
		        	$.ajax({
		              url:"<?php echo base_url(); ?>admin/suppression_university",
		              method:"POST",
		              data:{iduniv:iduniv},
		              success:function(data)
		              {
